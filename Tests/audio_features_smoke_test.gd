...
func _run() -> void:
	...
	assert(audio._dungeon_player.volume_db == GameAudio.SILENT_DB, "Dungeon music must begin silent")
	assert(audio._boss_player.volume_db == GameAudio.SILENT_DB, "Boss music must begin silent")
	assert(audio._grass_player.stream != null and audio._forest_player.stream != null and audio._desert_player.stream != null and audio._dungeon_player.stream == GameAudio.DUNGEON_TRACKS[0] and audio._boss_player.stream == GameAudio.BOSS_TRACK, "Every biome and boss playlist must be initialized")
	assert(audio._grass_player.stream_paused and audio._forest_player.stream_paused and audio._desert_player.stream_paused and audio._dungeon_player.stream_paused and audio._boss_player.stream_paused, "Biome and boss music must begin paused")
	...
	audio._set_active_biome(GameAudio.Biome.DUNGEON)
	assert(not audio._dungeon_player.stream_paused, "Dungeon music must use the same playlist behavior as overworld music")
	audio._on_dungeon_finished()
	assert(audio._dungeon_track_index == 1 and audio._dungeon_player.stream == GameAudio.DUNGEON_TRACKS[1], "Dungeon1 must advance to Dungeon2")
	audio._on_dungeon_finished()
	assert(audio._dungeon_track_index == 0 and audio._dungeon_player.stream == GameAudio.DUNGEON_TRACKS[0], "Dungeon2 must loop back to Dungeon1")
