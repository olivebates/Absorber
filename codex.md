...
- Input: left-click a painted floor tile to move the fox; click an enemy to chase it. Clicking a resource deposit opens its centered construction button; clicking a built producer opens capacity-building buttons on valid adjacent tiles. Clicking a fox NPC routes the player adjacent and opens its due dialogue or shop. Ground item drops are collected by stepping onto their tile; Randal's quest ball is instead collected through its explicit interaction after the quest begins. Click or press Space/Enter to advance dialogue. Shift+0-9 saves, Ctrl+0-9 loads, Shift+P fills every resource to capacity, Shift+O toggles the debug stat menu, and Tab opens the world map for campfire teleportation.
- Navigation: four-way `AStarGrid2D` with Jump Point Search enabled. It is built once when `World` enters the tree. Actor occupancy is maintained incrementally as actors cross cells, reused as per-frame mover snapshots, and expensive AI A* work is limited to two requests per physics frame; nearby enemies share a player-centered flow field before falling back to one target-and-truncate A* route.
- Combat: actors automatically attack enemies on a directly adjacent tile. Enemies stop patrolling when engaged. Enemy rewards are Damage, Health, or Resource and are granted only when their flying orb reaches its destination. Damage is stored in a six-weapon by three-enemy-color matrix, while collectible weapons add their exact merge-count-scaled bonus.
- HUD: opaque black backplates cover five tiles (320 pixels) inward from both sides and three tiles (192 pixels) inward from the top and bottom, behind every GUI control. Corner groups use 12-pixel sidebar insets and eight-pixel section spacing. Left-side texture icons render at their native 32x32 size; top-left values use 22px text while bottom-left resources retain 24px. Top-left stat containers use eight-pixel padding/spacing, damage/defense cells share standardized 40x40 dimensions, and primary Health/Mana cells are stronger than secondary regeneration cells. Discovered resources stack in full-width vertical rows. The fixed 296x207.2 minimap has a matching `Map` header containing Settings. Inventory and Equipment share one bottom-right frame, with the Quest Log embedded in the Inventory header and the six-column 42px slot rows beneath labeled sections. Item merges show shadowed, full-width grade-colored stat-upgrade strips centered 25% from the top; concurrent strips stack downward and reveal 0.2 seconds apart. Unlocked player/Asha skills occupy the bottom center. The 1280x720 viewport uses fractional scaling with `expand` aspect handling so fullscreen/window resizes reach the screen borders instead of jumping between integer multiples.
...
- `find_path_to_actor_adjacent()` performs one route search to an occupied actor and removes the occupied final tile, replacing four separate adjacent-tile searches.
- `get_flow_path_to_player_adjacent()` reuses one static, player-centered twelve-tile flow field for nearby pursuit and falls back safely when dynamic occupancy blocks its gradient.
- Navigation actors register once and synchronize only when crossing a cell. Their filtered occupied-cell dictionaries are cached per mover for the current physics frame, while opening gates, doors, chests, and runtime buildings update membership immediately.
- AI callers share a two-A*-request physics-frame budget. Deferred callers retry after their own staggered delay rather than creating a frame spike.
...
Pursuit replans are staggered by 100-200 milliseconds, reuse one occupancy snapshot per decision, prefer the shared player flow field, and use at most one target-and-truncate A* fallback. Non-boss overworld patrol enemies outside the camera view plus a six-tile margin batch their AI work at 0.2-second intervals; enemies on-screen or within that margin, along with combat, pursuit, hunter targets, bosses, and all dungeon enemies, retain full-rate behavior. Enemy population and the global physics tick rate are unchanged.
...
- Inventory and Equipment are labeled sections inside one content-fitted bottom-right card. Inventory, weapon, and armor item slots are 42x42 and wrap into six columns. New players start with four inventory slots. Equipment has six weapon and six armor positions; the first pair starts unlocked and the other five pairs start locked. A gold `Merge All` button spans the five columns beside the trash slot and repeatedly combines matching equipment and stones. The Quest Log, Bazaar, and discovered-Buildings buttons share the Inventory header area. Toolbar clicks return an unlocked item to inventory.
...
- Every completed equipment merge raises a full-screen-width strip containing the item icon and its actual old/new Damage or Armor value. The shadowed strip enters from the right at 25% screen height while the shadowed content enters from the left, holds fully visible for three seconds, then exits left while its content fades. Its background uses the resulting grade color (including animated Omnipotent color), multiple live strips stack downward, and near-simultaneous merges are staggered by 0.2 seconds.
- Item grades progress through Crude (gray), Ordinary (white), Superior (green), Elite (blue), Masterwork (purple), Mythic (orange), Divine (pink), Immortal (teal), animated-rainbow Omnipotent, and Void (black). Further merges are unlimited and display as `Void +1`, `Void +2`, and so on. Every grade has a matching background in inventory, toolbar, and enemy-drop cards; Omnipotent also animates player attack slashes. Equipment power is its base stat multiplied by its exact merge total, so every merged copy contributes even between grade thresholds.

- Six colorized stat stones provide +2 damage or defense in red, yellow, or blue per merged copy. Identical stones merge, and dragging one onto any equipment replaces its single nested stone; weaker replacements require a cursor-adjacent confirmation. Equipment tooltips show a bordered stone slot and item icons show the equipped stone at 8x8 in their lower-left corner. Blue Sword is a blue-tinted weapon with 10 base blue damage.

- Hovering within two tiles of an active shop's original spawn shows its current offers and prices. Retired NPC catalogs move into the saved Bazaar icon grid; its scales button appears after the first retirement and receives a green arrival dot for every retirement. Build buttons permanently discover their producer/capacity type in a saved Buildings gallery; gallery names omit their `Build ` action prefix, and Gold/Gem mine cards layer the originating deposit behind the mine sprite. The house button appears after the first discovery and receives a red dot from the build location. Both Bazaar and Buildings buttons grow and shrink when their arrival dot lands.
...
`skill_system_smoke_test.gd` verifies the simplified dungeon hover card, Bat/Millipede variants, spawn-configured enemy skill timing/telegraphs/cascade/tint, tutorial cooldown and mana preparation, player roll immunity and placement, Golden Guard, toolbar locks/icons, Asha's bar, chest skill rewards, zero-defense dungeon reset vitals, and skill/mana persistence. `charged_and_scatter_skills_smoke_test.gd` verifies Charged Strike's mana cost, full three-second delay, seven-times damage, and Scatter Strike's Inspector availability, one-second delay, fixed player target, unique two-tile random targets, and wall omission. `dungeon_reward_extensions_smoke_test.gd` also covers the forced swap tutorial, compact skill frames, immediate cursor-centered dragging, 24-pixel snapping, drag-only assignment, persistent multi-swap editing, and picker exit paths.

`enemy_performance_smoke_test.gd` verifies incremental actor-cell updates, per-frame occupancy reuse, the two-request path budget, player flow routes, target-and-truncate paths, staggered replans, full-rate enemy movement through the six-tile camera margin, distant patrol AI batching, constant-time target/adjacency lookups, and minimap texture reuse under the current 96-enemy world. `hud_edge_bars_smoke_test.gd` verifies the left/right five-tile and top/bottom three-tile black HUD backplates, their screen-edge anchoring, input passthrough, placement beneath the GUI controls, content-fitted corner panels, the fixed-width 30%-shorter minimap and its Settings header, native 32px left icons, full-width vertical resource rows with left-aligned values and a 32px amount/rate gap, primary/secondary vital styling, 22px top-left values, eight-pixel margins, standardized 40x40 damage/defense cells, the unified Quest/Inventory/Equipment card, six-column 42px item layout, four starting inventory slots, six equipment slots, locked added equipment positions, four-slot save migration, and non-overlapping side stacks.

`sep01_requested_changes_smoke_test.gd` covers merge totals and grade milestones, merge persistence, revised item stats/tooltips, full-health potion rejection, Quest Log/shop state, Show Enemies defaults, and helper item handoff persistence. `merge_upgrade_banner_smoke_test.gd` verifies actual old/new weapon and armor values, icons, grade colors, full-width layout at 25% screen height, opposing entrance motion, shadows, downward stacking, the 0.2-second stagger, three-second hold, left exit, content fade, and cleanup. `sep01_dungeon_load_smoke_test.gd` round-trips a live active-dungeon enemy's health and position and verifies restored cooldown bounds. `sep01_map_quest_visual_smoke_test.gd` verifies exact quest cross-out geometry, the active floor atlas's first three second-row colors, and black unpainted full-map coordinates. `sep01_quest_stats_skill_feedback_smoke_test.gd` verifies quest-location labels, opaque button styling, 50%-brightened icon sizing and inventory alignment, red-dot absorption feedback, stat hover names, and player-originating cooldown/mana failure text. `settings_menu_smoke_test.gd` also verifies fractional viewport scaling and that disk controls stay unavailable without a graphical Windows display.
...
- Fox NPCs roam within two tiles of their scene positions unless authored stationary, pause while approached, block navigation, and show a yellow hover outline. Before recruitment, FoxAsha's Store opens only after Mira reaches an adjacent tile and no story conversation is due. Its top rows sell one Fish or Wood for 2 Gold, followed by its stat upgrades; the former Fish-priced health upgrade is no longer offered. Buying Deru's Spare Cart Parts recruits Asha, permanently closes her Store, and turns her into Mira's healing follower. Her join moment locks gameplay while a text-height black banner slides in from the right beneath Asha, then pauses for 0.25 seconds between the portrait appearing, rotating sun rays fading in, and `Asha joins the party!` popping into view. `AshaJoins.mp3` begins with the banner while biome music quickly fades out; the title receives the camera punch. There are no explanatory lines below it. After at least three seconds a continue prompt appears, and the rays keep rotating until any key or mouse click closes the celebration and restores biome music. `Lios Shop` sells one Gold for 2 Fish, +4 red damage for 5 Fish, and +20 health for 5 Jewels. Chloe's Shop trades one Herb for 2 Cave Moss, one Cave Moss for 2 Wood, +4 blue damage starting at 5 Cave Moss, and its sole one-time inventory slot for 15 Cave Moss; no other shop sells inventory, equipment, or skill slots. Store buttons remain enabled and visibly highlight even when unaffordable. Inventory-style hover cards appear instantly, follow the pointer, and preview the current and resulting amount. Rows stagger into view, lift and enlarge their icon on hover, flash and send icons toward the HUD on success, or shake and show the missing cost on failure. Shopkeepers react to their first purchases, and the dialogue preserves the focused row and pointer when the Store reopens. The centered box fits visible rows with exact 8px content margins and closes via its top-right X, Escape, or an outside click.
- Giving the Spare Cart Parts to Deru repairs his cart and recruits him as a hunter instead of unlocking a shop. He uses Lio's invulnerable hunter loop against eligible AreaID 2 enemies and deals 7 damage per hit. Randal's interaction-only lost-ball quest adds the protected `Randal's Ball` inventory item. On its return, Randal asks how he can repay Mira, Mira proposes clearing creatures from the woods, and Randal agrees before joining the same helper loop for AreaID 5; he deals 17 damage per hit. Straight Jump Point Search routes retain their final approach tile so helpers move horizontally toward aligned enemies. Lio, Deru, and Randal carry stat and item drops, return to their area's campfire, and wait there for handoff. If Mira lacks room for every carried item, a two-inventory popup permits dragging items in either direction before confirmation; confirming applies Mira's chosen layout and clears the helper's carried items. Small stat handoffs are free; larger handoffs use the helper's configured fee. Hovering a helper who is waiting with rewards at a campfire opens the shared equipment-style tooltip, listing every carried reward followed by `Price: Free` or the helper's price. Hunter state, collected stat and item rewards, fee authorization, position, and repaired visuals persist in saves, including migration from older saves.
- The compact bottom dialogue panel fits the longest message in each conversation horizontally up to a readable wrapping limit, stays centered, and enters with a short slide-and-scale motion. Text reveals progressively with punctuation pauses; every revealed non-whitespace glyph replays `sndTalk` on the SFX bus with the active character's authored pitch range. Mira uses 0.9-1.1, female foxes use 1.3-1.5, and male foxes use 0.5-0.7. Skipping, closing, or cancelling dialogue stops the talk sound. The first advance completes the line and the next continues. A pulsing arrow replaces instructional copy after each line finishes, while the active portrait pops, brightens, and gently bobs. Mira's unflipped portrait and name appear on the left; NPC identity appears on the right with the portrait flipped toward the text, except Asha, whose portrait remains unflipped. All story foxes stop walking while it is open. Lio's first interaction introduces his work, adds `If only I had some way to automate it, hah!`, and begins the gold-mine quest; while that quest remains active, later talks give a one-line automation reminder and reopen his shop afterward. Asha's welcome runs only on her first direct interaction and opens the shop afterward. Mad Coyote's first snare pauses 0.2 seconds after its skill and gives skill-sensitive escape dialogue; its first defeat celebrates the path opening. Persistent remarks include a seven-tile cave-bull warning, the first Spider defeat's mana reminder, and a ChickenSpawn12 Evil Goat kill trigger.
- The Quest Log button has a fully opaque dark frame, uses `iconQuest.webp` at its native size with the sprite modulated 50% brighter, sits inside the Inventory header, and badges the number of active quests at bottom-right. Starting or advancing a quest sends a circular red dot from Mira into the button, which grows, contracts, and settles as it absorbs the update. Its popup closes by X, outside click, or Escape. Quest title buttons append their location in parentheses, expand through the current step, cross completed steps in red, and prefix completed quests with a green checkmark. Lio's Tiny Woods automation quest requires all three gold mines; Deru's Snakemouth Expanse quest requires Asha's cart part; Randal's Whippersnapper Woods quest requires interacting with and returning his ball.
...
- Enemy physics calculates player adjacency once per enemy tick, caches player/world/dialogue references, and uses an incrementally maintained world-level actor/enemy cell index instead of rescanning all actors. Per-mover occupancy snapshots are reused for the full physics frame. Player combat resolves adjacency from that spatial index once and reuses the result for attacking, facing, centering, and its combat ring. Patrol destinations are static-cached per spawn radius, shuffled without duplicate attempts, and filtered against current occupancy/reservations before making one budgeted path request. Pursuers use a shared twelve-tile player flow field, one occupancy calculation, a single target-and-truncate A* fallback, a two-request-per-frame global AI path budget, and 100-200ms staggered replans. Non-boss overworld patrol enemies outside the current camera rectangle plus a six-tile margin batch AI at 0.2-second intervals without reducing population or the global physics rate; the visible rectangle is calculated once per physics frame. Newly spawned enemies retain a randomized initial patrol delay. Full spawn markers do not request redraws while their respawn indicator is hidden. World/cell conversion uses the square 64-pixel grid directly. The minimap redraws at 10 Hz, caches nearby terrain into one tiny texture until exploration or its center cell changes, and reuses cell coordinates for markers and route dots; these changes keep large enemy populations and explored maps from producing frame loss or periodic redraw spikes.
...
- 2026-09-05 - Refined Randal's ball-return conversation so he first asks how to repay Mira, she suggests clearing the woods, and he agrees based on his familiarity with the forest.
- 2026-09-05 - Fixed straight Jump Point Search helper approaches, set Randal to 17 and retained Deru at 7 damage per hit, and rewrote Randal's ball-return exchange as a natural conversation.
- 2026-09-05 - Added Chloe and her Cave Moss/Wood shop, removed slot purchases from other shops, added Randal's interaction-only lost-ball quest and persistent AreaID 5 helper role, and followed the first producer-build remark with a one-time reminder that clicking it again exposes more construction.
- 2026-09-03 - Reopened Lio's shop after his active-quest reminder, removed `Build ` from gallery names, layered Gold/Gem deposits behind mine-card sprites, pulsed Bazaar/Buildings buttons when dots arrive, and renamed the top-left damage-icon hover to `Weapon Type`.
- 2026-09-03 - Standardized dialogue pitch variation to 1.3-1.5 for female foxes and 0.5-0.7 for male foxes while retaining Mira's 0.9-1.1 range.
- 2026-09-03 - Replayed `sndTalk` for each non-whitespace typewriter glyph, added per-character dialogue pitch ranges with neutral Mira, higher female foxes, and lower male foxes, and stopped voice audio on skips and dialogue closure.
- 2026-09-03 - Fixed clipped Building prices by reserving width for plain numeric amounts, and added a white bottom-aligned `x<count>` beside each building sprite once at least one matching structure has been constructed.
- 2026-09-03 - Made Building cards show explicit icon-led resource amounts while withholding effects until the first matching building exists; grouped Bazaar entries by cost resource and then price, and enlarged their bottom-left currency badges to 16px with readable cost text.
- 2026-09-03 - Expanded Buildings to four equal cards per row with a two-and-a-half-row viewport, removed its hover cards, expressed production in minutes, added bottom-left Bazaar cost badges, hid fogged shop markers/previews, and made recruited helper navigation unrestricted with immediate post-load path rebuilding.
- 2026-09-03 - Moved icon-rich two-tile shop previews onto the world map, added Bazaar stat/currency hover rows, placed Quest/Bazaar/Buildings controls above the Inventory title, rebuilt discovered-building cards with vertical icon costs and green production/capacity details, kept deposit sprites visible beneath mines, removed stones from stores, and changed loading to Ctrl+number.
- 2026-09-03 - Made every merged equipment copy add its base stat, forced merge-banner text white, added six mergeable color/stat stones with one-stone equipment sockets and weaker-replacement confirmation, added the blue 10-damage sword, reduced starting inventory to four, framed shop slot upgrades, added two-tile shop previews, retired-shop Bazaar purchasing, and persistent discovered-building gallery feedback.

- 2026-09-02 - Moved merge feedback to shadowed full-width strips at 25% screen height, with right-entering/left-exiting grade banners, left-entering centered content, a three-second full hold, and content fade during exit; retained actual previous-to-new stat values.

- 2026-09-02 - Added large center-screen item-merge upgrade banners with item icons, actual old/new Damage or Armor values, resulting-grade colors, downward stacking, and 0.2-second reveal staggering.

- 2026-09-02 - Changed distant enemy AI batching to begin only outside the actual camera-visible world rectangle plus a six-tile margin, keeping on-screen and near-screen enemy movement at full rate; added exact boundary regression coverage.

- 2026-09-02 - Implemented the remaining performance options without changing enemy population or the global physics tick: incremental actor occupancy with per-frame snapshots, immediate dynamic-blocker registration, one-search actor approach paths, a two-request AI path budget, 100-200ms staggered replans, a shared player flow field, and 0.2-second batching for distant non-boss overworld patrols; expanded performance regression coverage.

- 2026-09-02 - Removed severe high-enemy-count frame loss by caching minimap terrain as one texture, eliminating full-spawner redraw churn, using a per-frame spatial adjacency lookup and cached enemy dialogue reference, accelerating square-grid coordinate conversion, and caching/filtering unique patrol candidates before pathfinding; expanded the performance smoke test for adjacency and terrain-cache reuse.
